...
export default function RoomGallery({ images }: { images: string[] }) {
  ...
  return (
    <>
      ...
      {index !== null && (
        <div onClick={close} className="fixed inset-0 z-[9000] flex items-center justify-center p-6" style={{ background: "rgba(0,0,0,.92)", backdropFilter: "blur(6px)", animation: "lbIn .25s ease" }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <button onClick={close} className="absolute top-5 right-6 text-4xl" style={{ color: "var(--gold)" }} aria-label="Zamknij">×</button>
          {images.length > 1 && (
            <>
